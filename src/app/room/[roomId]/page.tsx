...
export default function RoomDetailPage() {
  ...
  if (authLoading || loadingRoom) {
    return (<div style={{ minHeight: "100vh", background: "var(--bg-base)" }}><div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh" }}><div className="spinner" /></div></div>);
  }
  if (!room) {
    return (<div style={{ minHeight: "100vh", background: "var(--bg-base)", display: "flex", alignItems: "center", justifyContent: "center" }}><div className="empty-state"><h3>Room not found</h3><p>This room may have been deleted.</p><button className="btn-primary" style={{ marginTop: 16 }} onClick={() => router.push("/dashboard")}>Back to Dashboard</button></div></div>);
  }
  ...
  return (
    <DashboardLayout title={room.roomName}>
      <div className="page-enter">
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 16 }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 20, flexWrap: "wrap" }}>
                  <span style={{ fontSize: 13, color: "var(--text-muted)" }}>Teacher: {room.teacherName}</span>
                  <span style={{ fontSize: 13, color: "var(--text-muted)" }}>Created: {formatDate(room.createdAt)}</span>
                  <span style={{ fontSize: 13, color: "var(--text-muted)" }}>{room.participants?.length || 0} students</span>
                </div>
              </div>
              {isTeacher && (
                <div onClick={copyCode} style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 16px", background: "var(--blue-light)", border: "1px solid rgba(59,130,246,0.15)", borderRadius: "var(--radius-md)", cursor: "pointer" }}>
                  <div>
                    <p style={{ fontSize: 11, color: "var(--text-muted)", marginBottom: 2 }}>ROOM CODE</p>
        ...
        {/* Recordings */}
        <h2 style={{ fontSize: 18, fontWeight: 600, color: "var(--text-primary)", marginBottom: 16 }}>Recordings ({recordings.length})</h2>
        {recordings.length === 0 ? (
          <div className="card empty-state">
            ...
            <p>{isTeacher ? "Record your classes or add recording links" : "Your teacher will add recordings after sessions"}</p>
          </div>
        ) : (
