...
export default function AttendancePage() {
  ...
  if (!room) return (<div style={{ minHeight: "100vh", background: "var(--bg-base)", display: "flex", alignItems: "center", justifyContent: "center" }}><div className="empty-state"><h3>Room not found</h3><button className="btn-primary" style={{ marginTop: 16 }} onClick={() => router.push("/dashboard")}>Dashboard</button></div></div>);
  if (!isTeacher) return (<div style={{ minHeight: "100vh", background: "var(--bg-base)", display: "flex", alignItems: "center", justifyContent: "center" }}><div className="empty-state"><h3>Access Restricted</h3><p>Only the teacher can view attendance.</p><button className="btn-primary" style={{ marginTop: 16 }} onClick={() => router.push(`/room/${roomId}`)}>Back</button></div></div>);

  const stats = [
    { label: "Students", value: totalStudents, color: "var(--blue)", bg: "var(--blue-light)" },
    { label: "Teachers", value: totalTeachers, color: "var(--yellow)", bg: "var(--yellow-light)" },
    { label: "Sessions", value: totalSessions, color: "var(--green)", bg: "var(--green-light)" },
    { label: "Avg Duration", value: formatDuration(avgDuration), color: "var(--purple)", bg: "var(--purple-light)" },
  ];
    ...
    <DashboardLayout title="Attendance">
      <div className="page-enter">
        ...
        <div style={{ marginBottom: 28 }}>
          <h1 style={{ fontSize: 26, fontWeight: 700, marginBottom: 4 }}>Attendance Records</h1>
          <p style={{ fontSize: 14, color: "var(--text-secondary)" }}>Track who attended meetings in {room.roomName}</p>
        </div>
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 24, flexWrap: "wrap", alignItems: "center" }}>
          <div style={{ flex: 1, minWidth: 200, position: "relative" }}>
            ...
            <input type="text" placeholder="Search by name..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="input-field" style={{ paddingLeft: 36 }} />
          </div>
          <div style={{ display: "flex", gap: 6 }}>
            {(["all","student","teacher"] as const).map(f => (
              <button key={f} onClick={() => setFilterRole(f)} style={{
                padding: "8px 16px", borderRadius: "var(--radius-full)",
                border: `1px solid ${filterRole === f ? "var(--blue)" : "var(--border-light)"}`,
                background: filterRole === f ? "var(--blue-light)" : "transparent",
                color: filterRole === f ? "var(--blue)" : "var(--text-secondary)",
        ...
        {filteredAttendees.length === 0 ? (
          <div className="card empty-state"><h3>No attendance records</h3><p>{searchTerm ? "No matching records." : "Records will appear when participants join meetings."}</p></div>
        ) : (
          <div className="card" style={{ padding: 0, overflow: "hidden" }}>
            <div className="attendance-table-wrap">
              <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr 1fr", padding: "14px 20px", background: "var(--bg-elevated)", borderBottom: "1px solid var(--border)", fontSize: 12, fontWeight: 600, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.5px", gap: 12 }}>
                <span>Participant</span><span>Role</span><span>Last Join</span><span>Sessions</span><span>Total Time</span>
              </div>
              {filteredAttendees.map((record, index) => (
                <div key={record.id} style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr 1fr", padding: "14px 20px", borderBottom: index < filteredAttendees.length - 1 ? "1px solid var(--border)" : "none", alignItems: "center", gap: 12, transition: "background var(--transition-fast)" }}
                  ...
                  <span className={`badge ${record.userRole === "teacher" ? "badge-teacher" : "badge-student"}`}>{record.userRole}</span>
                  ...
                  <span style={{ fontSize: 14, fontWeight: 500, color: getTotalDuration(record.userId) > 0 ? "var(--green)" : "var(--text-muted)" }}>
                    {getTotalDuration(record.userId) > 0 ? formatDuration(getTotalDuration(record.userId)) : "In progress"}
                  </span>
                </div>
              ))}
            </div>
        ...
        {records.length > 0 && (
          <div style={{ marginTop: 36 }}>
            <h2 style={{ fontSize: 18, fontWeight: 600, color: "var(--text-primary)", marginBottom: 16 }}>Session Log ({records.length})</h2>
              ...
              {records.map(record => (
                <div key={record.id} className="card" style={{ padding: "14px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <span style={{ fontSize: 14, fontWeight: 500, color: "var(--text-primary)" }}>{record.userName}</span>
                    <span className={`badge ${record.userRole === "teacher" ? "badge-teacher" : "badge-student"}`} style={{ fontSize: 10, padding: "2px 8px" }}>{record.userRole}</span>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
                    ...
                    {record.leaveTime ? (
                      ...
                      <span style={{ fontSize: 12, fontWeight: 600, color: "var(--green)", background: "var(--green-light)", padding: "3px 10px", borderRadius: "var(--radius-full)" }}>● Active</span>
                    )}
                  </div>
                </div>
              ))}
